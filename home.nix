...

{
  ...
  # ═══════════════════════════════════════════════════════════════════════
  # Accent is hardcoded to "blue" as the system default for catppuccin/nix
  # modules (GTK, starship, delta, brave). The visible accent (Hyprland
  # borders, Waybar) is managed imperatively by the wallpaper script via
  # writable runtime files in ~/.config/catppuccin/ — no rebuild needed.
  catppuccin = {
    enable = true;
    autoEnable = true;
    flavor = "mocha";
...
BEMENU_EOF
    '';
    # Bootstrap Catppuccin runtime state directory with default (blue) accent.
    # These files are writable at runtime — the wallpaper script updates them
    # live without needing a rebuild. Only created if they don't already exist.
    bootstrapCatppuccin = ''
      mkdir -p "$HOME/.config/catppuccin"
      if [ ! -f "$HOME/.config/catppuccin/current-accent" ]; then
        echo -n "blue" > "$HOME/.config/catppuccin/current-accent"
      fi
      if [ ! -f "$HOME/.config/catppuccin/current-wallpaper" ]; then
        echo -n "rheinfall.jpg" > "$HOME/.config/catppuccin/current-wallpaper"
      fi
      if [ ! -f "$HOME/.config/catppuccin/hyprland-accent.conf" ]; then
        cat > "$HOME/.config/catppuccin/hyprland-accent.conf" <<'HYPR_EOF'
# Generated by home-manager activation — wallpaper script will update this
# Sourced by hyprland.conf via: source = ~/.config/catppuccin/hyprland-accent.conf
decoration {
    col.active_border = rgba(89b4faff)
}
HYPR_EOF
      fi
      if [ ! -f "$HOME/.config/catppuccin/waybar-accent.css" ]; then
        cat > "$HOME/.config/catppuccin/waybar-accent.css" <<'CSS_EOF'
/* Generated by home-manager activation — wallpaper script will update this
 * Imported by waybar/style.css via: @import url("file:///home/gc/.config/catppuccin/waybar-accent.css");
 */
